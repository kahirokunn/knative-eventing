...
func ChannelToSink() *feature.Feature {
	...
	f.Requirement("install sender", eventshub.Install(
		senderName,
		eventshub.StartSenderToResource(channel.GVR(), channelName),
		eventshub.InputEvent(event),
	))

	f.Assert("receiver rejects the first three deliveries", assert.OnStore(receiverName).
		MatchRejectedEvent(cetest.HasId(event.ID())).Exact(3))
	f.Assert("receiver accepts the fourth delivery", assert.OnStore(receiverName).
	...
	f.Assert("retry delay stops growing at two seconds", assert.OnStore(receiverName).
		Match(deliveriesFollowBackoff(event.ID(), []time.Duration{time.Second, 2 * time.Second, 2 * time.Second})).Exact(4))
...
func deliveriesFollowBackoff(id string, expected []time.Duration) eventshub.EventInfoMatcher {
	var mu sync.Mutex
	seen := make(map[uint64]eventshub.EventInfo, len(expected)+1)
